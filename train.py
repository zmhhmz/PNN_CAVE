...
import numpy as np
import os
#from others import interp23, saveLayer
import scipy.io as sio
import scipy.misc as misc
from utils import interp23, saveLayer, down_img, input_prep
...
param={
    ...
    'layers':[],
    'train_dir':'train_dir/',
    'save_model_name':'PNN_model1.mat',
    'cost':'L1',
    'residual':True,
    'regol':True,
    'ratio':32,
...
def train():
    if len(param['layers'])==0:
        ...
        layers=[]
        for i in range(0,len(param['layers']),2):
            layers.append(ConvLayer(param['layers'][i], param['layers'][i+1]))

    net = Network(layers)

    allX, allY = Crd.all_train_data_in()

    for j in range(0,param['epoch']):  #start point
